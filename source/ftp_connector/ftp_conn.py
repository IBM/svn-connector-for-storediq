...
class FtpConnector(TemplateConnection):
    # Enable for local logging. Use appropriate log file path
    _mylogger = logging.getLogger("FtpConnector")
    ...
    def __init__(self, serverName, userName, userPwd, options, serial, mountPoint):
        """
        Initialize the FtpConnector connection.
        """
        self._mylogger.info('FtpConnector.__init__(): servname=%s, uname=%s, pwd=%s, options=%s, serial=%s, mountPoint=%s' % (str(serverName), str(userName), str(userPwd), str(options), str(serial), str(mountPoint)))
        ...
        # if not self._share:
        #     # self._mylogger.info("FtpConnector.__init__(): servname=%s, uname=%s, options=%s: 'share' missing in options" % (str(serverName), str(userName), str(options)))
        #     # return
        ...
        # self._mylogger.info("FtpConnector.__init__(): _mp=%s" % (str(self._mp)))
    ...
    def get_attribute_def(cls):
        ...
        '''
        import ftp_connector.sample_attributes as attr
    ...
    def get_fs_name(cls):
        ...
        '''
        from ftp_connector.sample_attributes import fs_name
        return fs_name
    ...
    def _fqn(self, path):
        ...
        '''
        fpath = path
        self._mylogger.info('1) FtpConnector.__fqn(): path=%s' % str(fpath))
        # self._mylogger.info('2) FtpConnector.__fqn(): self._mp=%s' % str(self._mp))
    ...
    def connect(self):
        ...
        """
        self._top_level = True
        # self._mylogger.info('FtpConnector.connect(): serverName: %s, userName: %s'  % (self._server_name, self._userName))

        if not self._mp:
            # self._mylogger.info('FtpConnector.connect(): Unknown Mount-point: serverName: %s, share: %s'  % (self._server_name, self._share))
            return False
        ...
        if not os.path.ismount(self._mp):
            ...
            # self._mylogger.info('FtpConnector.connect(): %s, already mounted' % (self._mp))
            # TODO: Unmount only if mounted. Ignore error for now.
            return True
        self._top_level = True
        mount_point = '/deepfs/{0}'.format(self._server_name)
    ...
    def disconnect(self):
        ...
        """
        self._mylogger.info('FtpConnector.disconnect:')
        if self._mp:
            errorcode, output = commands.getstatusoutput('/bin/umount %s' % self._mp)
            self._mylogger.info('FtpConnector.disconnect(): umount %s, status=%s, output=%s' % ( self._mp, str(errorcode), output))
            self._mp = None
    ...
    def shutdown(self):
        ...
        """
        self._mylogger.info('FtpConnector.shutdown:')
        self.disconnect()
        return True
    ...
    def is_read_only(self, path):
        ...
        """
        #self._mylogger.info('FtpConnector.isReadOnly(): path=%s' % path)
        st = self.lstat(path)
        mode = st[ST_MODE]
    ...
    def create_checkout(self, path):
        mount_point = '/deepfs/{0}'.format(str(self._server_name))
        local_checkout = '{0}/{1}'.format(mount_point, path)
        #self._mylogger.info('FtpConnector.checkout_mount(): full_url: %s local_checkout: %s'  % (full_url, local_checkout))
        #test code snippet
        if not os.path.exists(local_checkout):
            os.makedirs(local_checkout)
        #ends here
        self._mylogger.info('FtpConnector.checkout_mount(): CheckOUT Started')
        remote_dir = path
            ...
            for item in dir1:
                try:
                    if type(ftp.size(item)) == int:
                        with open(local_checkout + '/' + item, 'wb') as f:
                            ftp.retrbinary("RETR {0}".format(item), f.write)
                except:
                    self.download_ftp_tree(ftp, item, local_checkout)
        else:
            self.download_ftp_tree(ftp, remote_dir, local_checkout)
        self._mylogger.info('FtpConnector.checkout_mount(): CheckOUT Complete')

        # Build mount command w/ arguments
        cmd = "/bin/mount --bind {0} {1}".format(mount_point, self._fqn(self._mp))
        errorcode, output = commands.getstatusoutput(cmd )
        self._mylogger.info('FtpConnector.checkout_mount(): cmd=%s, status=%s' % ( cmd, str(errorcode)))
        if errorcode:
            emsg = 'mount %s failed, status=%s, msg=%s' % (self._mp, str(errorcode), output)
            raise TemplateInvalidPathException
    ...
    def start_checkout(self, path):
        if self._volume_register:
            if not os.path.exists(self._fqn(path)):
    ...
    def list_dir(self,node):
        ...
        '''
        self._mylogger.info('FtpConnector.list_dir(): Entered >>>>>>>>>>>>LD')
        path = node.path
        extras = node.extras
        ...
        self._mylogger.info('FtpConnector.list_dir(): path=%s, extras=%s' % (path, str(extras)))
        if self._top_level:
        ...
        try:
            ...
            self._mylogger.info('1) FtpConnector.list_dir(): path=%s' % (str(self._fqn(path))))
            dir_entries = os.listdir(self._fqn(path))
            self._mylogger.info('2) FtpConnector.list_dir(): dir_entries=%s' % (str(dir_entries)))
        except IOError as e:
            #self._mylogger.info('FtpConnector.list_dir(): Error %s, path=%s, extras=%s' % (str(e), path, str(extras)))
            raise 'FtpConnector.list_dir(): Error "%s" on "%s"' % (e, path)

        self.retlist = []
        # When node is top level directory
        if not dir_entries:
            self._mylogger.info('FtpConnector.list_dir(): node_path=%s' % (str(node.path)))
            return self.list_dir_next(node)

        for item in dir_entries:
            #self._mylogger.info('FtpConnector.list_dir(): path=%s, ini_dir=%s, item=%s' % (path, self._initial_dir, item))
            ...
            #self._mylogger.info('FtpConnector.list_dir(): item_path=%s, lstat_path=%s' % (item_path, lstat_path))
    ...
    def list_dir_next(self, node):
        ...
        '''
        #self._mylogger.info('FtpConnector.list_dir_next(): path=%s, extras=%s' % (node.path, str(node.get_extras())))
    ...
    def lstat(self, path, extras={}):
        ...
        '''
        self._mylogger.info("FtpConnector.lstat(): path=%s, extras=%s" % (path, str(extras)))
        if not self._volume_register:
            self._mylogger.info("lsstat: volume register")
    ...
    def lstat_extras(self, path, extras={}):
        ...
        '''
        #self._mylogger.info('FtpConnector.lstat_extras(): path=%s, extras=%s' % (path, str(extras)))
    ...
    def _doesFileExist(self, requestPath):
        ...
        '''
        #self._mylogger.info("FtpConnector._doesFileExist(): requestPath=%s" % requestPath)
        try:
    ...
    def open_file(self, path, mode=TemplateConnection.FILE_READ):
        ...
        #self._mylogger.info('FtpConnector.open_file(): path=%s, mode=%s' % (path, str(mode)))

        if mode==TemplateConnection.FILE_READ:
            if not self._doesFileExist(path):
                raise IOError("FtpConnector.open_file: The file, %s cannot be opened." % (path))
            else:
                try:
            ...
            raise IOError("FtpConnector.open_file:Open mode %s is not supported." % (mode))
    ...
    def write_file(self, path, fileHandle, filebuf):
        ...
        """
        #self._mylogger.info("FtpConnector.write_file(): path=%s" % (path))

        if not fileHandle or not fileHandle.getOsHandle():
            raise IOError("FtpConnector.write_file: File %s, not open." % (path))

        if (fileHandle.mode != TemplateConnection.FILE_WRITE) :
            #self._mylogger.info("FtpConnector.write_file: file: '%s' is in mode: %s, write is not allowed" % (path, fileHandle.mode))
            raise IOError("FtpConnector.write_file: file: '%s' is in mode: %s, write is not allowed" % (path, fileHandle.mode))
        ...
        #self._mylogger.info('FtpConnector.write: input buffer count: %s' % (count))
        return w
    ...
    def read_file(self, path, fileHandle, readSize):
        ...
        #self._mylogger.info("FtpConnector.read_file(): path=%s, size=%s" % (path, readSize))
    ...
    def close_file(self, fileHandle):
        ...
        #self._mylogger.info('FtpConnector.close:  done')
        if not fileHandle.getOsHandle():
            raise IOError("FtpConnector.close: file not open.")
        try:
    ...
    def rmdir(self, path):
        ...
        Remove directory
        '''
        #self._mylogger.info('FtpConnector.rmdir():  path->%s< ' % (path))
        try:
            p = path
    ...
    def unlink(self, path):
        ...
        Deletes a file.
        '''
        #self._mylogger.info('FtpConnector.unlink:  path->%s< ' % (path))
        try:
            p = path
            if not path.startswith('/'):
                p = self._fqn(path)
    ...
    def validate_directories(self, path, startDir, endDir):
        ...
        '''
        #self._mylogger.info("FtpConnector.validate_directories(): path >%s< startDir: >%s< endDir: >%s<" % (path, startDir, endDir))
        ...
        #self._mylogger.info("FtpConnector.validate_directories(): idir=%s, sdir=%s, edir=%s" % (self._initial_dir, self._start_dir, self._end_dir))
    ...
    def verify_mount_in_filemgr(self):
        ...
        '''
        #self._mylogger.info("FtpConnector.verify_mount_in_filemgr():")
        return True
